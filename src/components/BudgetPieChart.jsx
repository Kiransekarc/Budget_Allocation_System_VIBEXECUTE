...
export default function BudgetPieChart({ allocations, income = 0 }) {
                ...
                <div className="bg-slate-900/90 backdrop-blur-md border border-slate-700 p-4 rounded-xl shadow-2xl">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-slate-300 flex justify-between gap-4">
                            ...
                            <span className="font-bold text-white">{data.value.toFixed(1)}%</span>
                        </p>
                    </div>
                </div>
            );
    ...
    return (
        ...
        >
            ...
            {totalPercent > 0 ? (
                <div className="w-full h-80 relative z-10"> {/* slightly increased height for center text to breathe */}
                    ...
                    {/* Centered Donut Text */}
                    <div className="absolute top-[45%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none pt-4"> {/* Adjusting center text slightly up since legend pushes pie down a bit */}
                        <p className="text-sm font-semibold text-slate-400">Total Uses</p>
                        <p className="text-3xl font-bold text-white drop-shadow-lg">{totalPercent.toFixed(1)}%</p>
                    </div>
                </div>
            ) : (
                <motion.div
                ...
                >
                    ...
                    <p className="font-medium text-lg tracking-wide">Assign values to see chart</p>
                </motion.div>
            )}
        </motion.div>
    );
