...
const CATEGORIES = [
    { id: 'rent', label: 'Rent', color: 'text-indigo-400', bg: 'indigo', shadow: 'rgba(129, 140, 248, 0.5)', thumbColor: '#818cf8' },
    { id: 'food', label: 'Food', color: 'text-emerald-400', bg: 'emerald', shadow: 'rgba(52, 211, 153, 0.5)', thumbColor: '#34d399' },
    { id: 'savings', label: 'Savings', color: 'text-amber-400', bg: 'amber', shadow: 'rgba(251, 191, 36, 0.5)', thumbColor: '#fbbf24' },
    { id: 'entertainment', label: 'Entertainment', color: 'text-rose-400', bg: 'rose', shadow: 'rgba(251, 113, 133, 0.5)', thumbColor: '#fb7185' },
    { id: 'utilities', label: 'Utilities', color: 'text-fuchsia-400', bg: 'fuchsia', shadow: 'rgba(232, 121, 249, 0.5)', thumbColor: '#e879f9' }
];
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3 self-start xl:self-auto bg-slate-800/50 p-1.5 rounded-lg border border-slate-700/50 min-w-0 max-w-full overflow-hidden">
                                    ...
                                    <div className="relative flex items-center min-w-0">
                                        ...
                                        <input
                                            ...
                                            className="pl-7 pr-2 py-1.5 text-right font-medium text-white glass-input rounded-md disabled:opacity-50 no-scrollbar w-24 min-w-0"
                                        />
                                    </div>
                                    <span className={`text-xs font-bold uppercase tracking-wider shrink-0 ${incomeNum <= 0 ? 'text-slate-700' : 'text-slate-500'}`}>or</span>
                                    {/* Percentage Input */}
                                    <div className="flex items-center min-w-0">
                                        <input
                                            ...
                                            placeholder="0"
                                            disabled={incomeNum <= 0}
                                            className="px-2 py-1.5 text-right font-medium text-white glass-input rounded-md disabled:opacity-50 no-scrollbar w-16 min-w-0"
                                        />
                                        <span className={`text-sm font-bold ml-1 mr-1 shrink-0 ${incomeNum <= 0 ? 'text-slate-600' : 'text-slate-400'}`}>%</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={`relative pt-1 ${incomeNum <= 0 ? 'opacity-50 grayscale' : ''}`}>
                                <input
                                    ...
                                    onChange={(e) => onAllocationChange(category.id, Number(e.target.value))}
                                    disabled={incomeNum <= 0}
                                    style={{ '--thumb-color': category.thumbColor }}
                                    className={`w-full h-2 rounded-lg appearance-none absolute top-1/2 -translate-y-1/2 z-10 slider-thumb-colored ${incomeNum <= 0 ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                                />
